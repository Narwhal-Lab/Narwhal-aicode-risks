...
2026年3月，随着大语言模型在编程领域的深度渗透，学术界对 AI 代码质量的担忧终于被确凿的数据证实。佐治亚理工学院系统软件与安全实验室（SSLab）通过其主导的“Vibe Security Radar”项目发布了严厉警告：仅在 2026 年 3 月这一个月内，就有至少 35 个新公开的 CVE（通用漏洞披露）漏洞，被确证是直接由 AI 代码生成工具引入的。详细报道如下：![信息安全杂志](./assets/image1.png)
...
​	这个 2026 年的真实案例完美印证了这一预判。由于 Agent 具备了“系统执行者”的权限，AI 生成的 react-codeshift 虚假依赖不再只是一行静止的文本，而是变成了 CI/CD 流水线中直接被拉取执行的配置文件。这种从“辅助”到“自主执行”的跃升，使得风险的破坏力呈指数级放大。
...
​	在 **4.2 节 编程语言分布** 里，报告通过数据揭示，AI 生成代码的渗透率在不同技术栈中存在显著差异。其中，拥有海量开源生态的 Python、JavaScript 和 TypeScript 占据了绝对主导地位 。这两个引发数万次真实下载的投毒测试案例，恰好精准地发生在 Python（pip install huggingface-cli）和 JavaScript/TypeScript（npm install react-codeshift）这两个生态中。这印证了报告的结论：高渗透率往往意味着潜在的高风险暴露面 。
...
**参考来源**
1. [InfoSecurity Magazine: Security Researchers Sound the Alarm on Vulnerabilities in AI-Generated Code](https://www.infosecurity-magazine.com/news/ai-generated-code-vulnerabilities/)
2. [Georgia Tech SSLab: Vibe Security Radar Project News](https://scp.cc.gatech.edu/external-news/infosecurity-magazine-security-researchers-sound-alarm-vulnerabilities-ai-generated)