# Moltbook RLS Data Exposure — Vibe Coding & Agent Architecture (2026)
> Moltbook 数据库泄露事件:Vibe Coding 文化与 Agent 架构默认值的双重失效

| Field | Value |
|---|---|
| Category | Human Factors (with Agent-Risk overlap) |
| Severity | 🔴 Critical |
| AI Tool | OpenClaw framework + AI-generated implementation |
| Language | TypeScript, SQL |
| Real Incident | ✅ |
| Reproducible | ✅ (anonymized PoC in body) |
| Disclosed | 2026-01-31 (404 Media), 2026-02-02 (Wiz Research) |
| CVE | — |
| CVSS | — |
| Primary sources | Wiz Research, 404 Media |

## TL;DR
Founder publicly stated they wrote no code — AI did the implementation. Result: 1.5M API tokens + 35K emails exposed via missing RLS. Frames Moltbook as a security-culture/skill-erosion case rather than a tooling bug.

> 创始人公开表示自己没写代码、全部交给 AI 实现;Wiz 研究人员证实 Supabase RLS 缺失导致 150 万 API Token 与 3.5 万邮箱可被未授权读取。本案例从 "安全文化侵蚀 / 工程师能力退化" 角度切入(与同期 Moltbook 智能体架构案例形成互补)。
...
## 5b. Agent 架构作为贡献因素 / Agent architecture as a contributing factor

> 本节合并自原 `2026-agent-architecture-bias-db` 案例 —— 验证后发现两份案例覆盖同一起 Moltbook 事件,故合并。

Moltbook 后端(数据层缺 RLS)是直接技术原因,但**底层 AI Agent 架构**为这种"默认乐观信任"留出了制度空间。Moltbook 自身基于 Peter Steinberger 的开源 AI Agent 框架 **OpenClaw**(原名 Clawdbot → Moltbot → OpenClaw,已在 NVIDIA 开发者博客和 Wikipedia 有词条)。

OpenClaw 设计上的一些默认值放大了风险:

- **默认信任本地连接**:大多数部署位于反向代理之后,所有连接看起来都来自 `127.0.0.1`,被视为可信流量,不需要身份验证。这种设计让 Agent 之间的横向信任成为默认。
- **高权限默认**:OpenClaw 默认授予 AI 智能体很高的权限,包括直接操控本地文件系统、执行 Shell 命令、调用邮件/日历等第三方服务,缺乏操作分级和权限隔离。
- **凭证管理混乱**:API 密钥、OAuth 令牌等敏感凭证在某些早期版本中被默认以明文形式存储在本地目录。

把这些默认值放在 Moltbook 这种"由 AI 快速实现 + 公开运行"的平台上,加上数据层 RLS 缺失,**就形成了"应用层信任默认开 + 数据层访问控制默认关"的双重失效**。Andrej Karpathy 在 X 上对 Moltbook 的态度从最初的赞叹("这是最接近科幻片中智能爆发的事物")迅速转变为警告("dumpster fire,绝对不建议在自己的电脑上运行,我把它放在隔离环境里跑都很害怕"),反映的正是这种系统性架构风险被现实验证的过程。

**启示**:对 AI Agent 平台的安全治理,不能只盯着 prompt 注入或工具滥用;还要审计底层框架的"信任默认值",并把数据层访问控制(RLS、对象级授权、租户隔离)作为强制基线。
