# Fake Claude Code Install Pages & Bing-AI-Promoted Malicious OpenClaw Repos (2026)
...
| Severity | 🟠 High |
| AI Tool | Claude Code (impersonated), Bing AI Search |
...
## TL;DR
Two parallel attacks (Mar 2026): Google Ads serving a fake Claude Code install page (Push Security calls it `InstallFix`); and Bing AI directly recommending a malicious GitHub org spoofing OpenClaw — delivering infostealers + GhostSocks.

> 攻击者用 Google Ads 投放与 Anthropic 文档高度相似的伪造 Claude Code 安装页,只替换一行命令(InstallFix);Huntress 同期披露 Bing AI 直接把伪造的 OpenClaw GitHub 组织作为安装入口推荐给用户,投递信息窃取器与 GhostSocks。
...
从攻击收益看，这类事件的价值远高于普通账号密码窃取。Claude Code、OpenClaw 等工具被安装和运行的主机，往往同时保存浏览器会话、密码管理器信息、SSH Key、开发者令牌、云平台访问凭证、API Key、本地源代码和测试环境配置。**具体投递的恶意软件家族**:Push Security 在 InstallFix 事件中确认投递的是 **Amatera Stealer**;Huntress 在 OpenClaw 事件中确认 Windows 端投递 **Vidar + GhostSocks**(后者将受害主机变为 SOCKS5 代理节点),macOS 端投递 **AMOS (Atomic macOS Stealer)**。Huntress 进一步指出，OpenClaw 运行环境中还可能存在高价值的 AI Agent 配置、自动化脚本和敏感工作流上下文，因此主机一旦被感染，损失不只体现在个人账号，而可能扩展到开发环境、代码仓库和企业 SaaS 访问链。
...
3. Anthropic 官方 Claude Code 安装文档(canonical 已迁至 code.claude.com)
